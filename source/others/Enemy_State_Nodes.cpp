...
EnemyDashOnFloor::EnemyDashOnFloor()
{
	timer.set_one_shot(true);
	timer.set_wait_time(0.5f);
	timer.set_on_timeout([&]()
...
void EnemyDashOnFloor::on_enter()
{
...
void  EnemyDashOnFloor::on_update(float delta)
{
	Enemy* enemy = (Enemy*)CharacterManager::instance()->get_enemy();
...
{
	MessageBox(GetHWnd(), _T("Well done!"), _T("挑战成功！"), MB_OK);
	exit(0);
}
...
void  EnemyRunState::on_update(float delta)
{
	...
	const Vector2& pos_palyer = CharacterManager::instance()->get_player()->get_position();
	enemy->set_velocity({ pos_enemy.x - pos_palyer.x ? SPEED_RUN: -SPEED_RUN,0 });

	if (enemy->get_hp() <= 0)
		enemy->switch_state("dead");
	else if (abs(pos_enemy.x - pos_palyer.x) <= MIN_DIS)
	{
		int rand_num = range_random(0, 100);
		if (enemy->get_hp() > 5)
		{
